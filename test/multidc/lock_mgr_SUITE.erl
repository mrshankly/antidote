...
-export([simple_transaction_tests_with_locks/1,
    ...
    asynchronous_test_one/1,
    asynchronous_test_two/1,
    asynchronous_test_three/1,
    asynchronous_test_four/1,
...
all() -> [
    ...
    asynchronous_test_one,
    asynchronous_test_two,
    asynchronous_test_three,
    asynchronous_test_four,
...
get_lock_owned_by_other_dc_one(Config) ->
    Clusters = proplists:get_value(clusters, Config),
    ...
    ok.
%% Test if sequential lock acquisition works as intendet
get_lock_owned_by_other_dc_two(Config) ->
    Clusters = proplists:get_value(clusters, Config),
    [Node1, Node2, Node3 | _Nodes] =  [ hd(Cluster)|| Cluster <- Clusters ],
...
%% 30 ms delay between increments
asynchronous_test_three(Config) ->
Clusters = proplists:get_value(clusters, Config),
    [Node1, Node2, Node3 | _Nodes] =  [ hd(Cluster)|| Cluster <- Clusters ],
...
a_lot_of_locks_per_transaction_one(Config) ->
    Clusters = proplists:get_value(clusters, Config),
    [Node1, Node2, Node3 | _Nodes] =  [ hd(Cluster)|| Cluster <- Clusters ],
    % asynchronous_test_4
...
a_lot_of_locks_per_transaction_two(Config) ->
    Clusters = proplists:get_value(clusters, Config),
    [Node1, Node2, Node3 | _Nodes] =  [ hd(Cluster)|| Cluster <- Clusters ],
    % asynchronous_test_4
...
cluster_failure_test_one(Config) ->
    Clusters = proplists:get_value(clusters, Config),
    [Node1, Node2, Node3 | _Nodes] =  [ hd(Cluster)|| Cluster <- Clusters ],
...
cluster_failure_test_two(Config) ->
    Clusters = proplists:get_value(clusters, Config),
    [Node1, Node2, Node3 | _Nodes] =  [ hd(Cluster)|| Cluster <- Clusters ],
    case rpc:call(Node1, application, get_env, [antidote, enable_logging]) of
        {ok, false} ->
